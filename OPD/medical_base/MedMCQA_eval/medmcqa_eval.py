"""Single-file MedMCQA evaluation script.

Example:
python OPD/medical_base/MedMCQA_eval/medmcqa_eval.py --visible-gpus 0 --record-file

You can also summarize a result file that already contains model_answer_idx:
python OPD/medical_base/MedMCQA_eval/medmcqa_eval.py --eval-only --data-paths result.jsonl
"""
...
DEFAULT_MODEL_PATH = "/home2/zc/workspace/xyuan/verl-OPD/OPD/model/Qwen/Qwen3-0.6B"
DEFAULT_DATA_PATHS = ["/home2/zc/workspace/xyuan/verl-OPD/OPD/data/medical/benchmark/MedMCQA.json"]
MEDMCQA_OPTION_KEYS = ("opa", "opb", "opc", "opd")
MEDMCQA_ANSWER_MAP = {1: "A", 2: "B", 3: "C", 4: "D"}


def build_medmcqa_messages(question: str, options: dict[str, str]) -> list[dict[str, str]]:
    """Build chat messages for a MedMCQA single-choice question."""
...
def parse_medmcqa_answer(response: str | None, options: dict[str, str]) -> str:
...
def normalize_medmcqa_item(item: dict[str, Any]) -> dict[str, Any]:
    """Normalize a MedMCQA item to the common question/options/answer_idx shape."""
    if "options" in item and "answer_idx" in item:
        item["options"] = {str(key).upper(): str(value) for key, value in item["options"].items()}
        item["answer_idx"] = str(item["answer_idx"]).upper()
        return item

    missing = [key for key in ("question", "cop", *MEDMCQA_OPTION_KEYS) if key not in item]
    if missing:
        raise KeyError(f"MedMCQA item is missing required keys: {missing}")

    cop = item["cop"]
    try:
        answer_idx = MEDMCQA_ANSWER_MAP[int(cop)]
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(f"Unsupported MedMCQA cop value: {cop!r}") from exc
...
def load_medmcqa_data(path: str) -> list[dict[str, Any]]:
    """Load MedMCQA data from a JSON array, JSON with a data field, or JSONL."""
    ...
    try:
        ...
        if isinstance(obj, list):
            return [normalize_medmcqa_item(item) for item in obj]
        if isinstance(obj, dict) and "data" in obj and isinstance(obj["data"], list):
            return [normalize_medmcqa_item(item) for item in obj["data"]]
    ...
    for line in raw_text.splitlines():
        ...
        if not line or line.lstrip().startswith("//"):
            continue
        data.append(normalize_medmcqa_item(json.loads(line)))
    return data
...
def vote_answers(responses: list[str], options: dict[str, str]) -> tuple[str, list[str | None]]:
    ...
    for response in responses:
        try:
            answer = parse_medmcqa_answer(response, options)
...
def evaluate_one_file(
    ...
    """Generate, parse, optionally save, and summarize one MedMCQA file."""
    data = load_medmcqa_data(path)
    conversations = [build_medmcqa_messages(item["question"], item["options"]) for item in data]
    ...
    for idx, item in enumerate(data):
        ...
        try:
            if vote_num > 1:
                ...
                item["model_answer_idx"] = parse_medmcqa_answer(item["model_response"], item["options"])
        except Exception:
...
def run_generation_eval(args: argparse.Namespace) -> None:
    """Load the model and run MedMCQA evaluation for all data files."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="MedMCQA vLLM generation and accuracy evaluation script")
    parser.add_argument(
        "--data-paths",
        nargs="+",
        default=DEFAULT_DATA_PATHS,
        help=f"One or more MedMCQA JSON/JSONL file paths. Defaults to {DEFAULT_DATA_PATHS[0]}",
    )
    parser.add_argument(
        "--model-path",
